...
describe('Windows packaging', () => {
  ...
  it('prepares and refreshes the stable Chrome extension folder on every launch', () => {
    ...
    expect(content).toContain('function renderPageReloadRequired()')
    expect(content).toContain("el('button', 'btn2 accent2', 'Reload page')")
    expect(content).toContain("ping && ping.error === 'extension'")
    expect(content).not.toContain("analyzeRes.error === 'not-running' || analyzeRes.error === 'extension'")
